...
public class SettlersOfKTaNEScript : MonoBehaviour
{
    ...
    void Awake()
    {
        foreach (KMSelectable street in Streets)
        {
            ...
            street.OnInteract += delegate () { StreetPress(pressedObject); return false; };
        }
    ...
    void PressHexa(int z)
    {
            ...
            {
                Debug.LogFormat("[Settlers Of KTaNE #{0}] STRIKE due to: No claimable ressources.", moduleId);
                Strike(Hexas[z]);
            }
    ...
    void PressCylinder(KMSelectable cylinder)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    Strike(cylinder);
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] STRIKE due to: Not enough ressources for a city. You had {1} ore and {2} grain.", moduleId, ore, grain);
                }
            }
        }
    }
    ...
    void StreetPress(KMSelectable street)
    {
            ...
            else
            {
                ...
                {
                    Strike(street);
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] STRIKE due to: Not enough ressources for a street. You had {1} wood and {2} brick.", moduleId, wood, brick);
                }
            }
    }
    ...
    void longStreet(Settlement x, int iteration, int prev) {                      //0 = LEFT; 1 == MIDDLE; 2 == RIGHT               //Exeption at getLeft/getRight
        if (iteration == 6)                                                       //done
        {
            points = points + 2;
        ...
        BrickDisp.AddInteractionPunch();
            ...
            else
            {
                if (CommandQueue.Length == 1 && tradeEnough(CommandQueue) && trade)
                {
                    ...
                    giveMats("B");
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] You traded 4 {1} for 1 {2}.", moduleId, getFullName(CommandQueue), getFullName("B"));
                    CommandQueue = "";
                    UpdateMats();
                    trade = false;
        ...
        if (thiefMode)
            ...
            else
            {
                if (CommandQueue.Length == 1 && tradeEnough(CommandQueue) && trade)
                {
                    ...
                    giveMats("W");
                    CommandQueue = "";
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] You traded 4 {1} for 1 {2}.", moduleId, getFullName(CommandQueue), getFullName("W"));
                    UpdateMats();
                    trade = false;
                    return;
                }
        ...
        if (thiefMode)
            ...
            else
            {
                if (CommandQueue.Length == 1 && tradeEnough(CommandQueue) && trade)
                {
                    SubtractMats(CommandQueue);
                    giveMats("O");
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] You traded 4 {1} for 1 {2}.", moduleId, getFullName(CommandQueue), getFullName("O"));
                    CommandQueue = "";
                    UpdateMats();
                    trade = false;
                    return;
        ...
        GrainDisp.AddInteractionPunch();
            ...
            else
            {
                if (CommandQueue.Length == 1 && tradeEnough(CommandQueue) && trade)
                {
                    SubtractMats(CommandQueue);
                    giveMats("G");
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] You traded 4 {1} for 1 {2}.", moduleId, getFullName(CommandQueue), getFullName("G"));
                    CommandQueue = "";
                    UpdateMats();
                    trade = false;
                    return;
        ...
        WoolDisp.AddInteractionPunch();
            ...
            else
            {
                if (CommandQueue.Length == 1 && tradeEnough(CommandQueue) && trade)
                {
                    SubtractMats(CommandQueue);
                    giveMats("S");
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] You traded 4 {1} for 1 {2}.", moduleId, getFullName(CommandQueue), getFullName("S"));
                    CommandQueue = "";
                    UpdateMats();
                    trade = false;
                    return;
    ...
    void PressDice()
    {
            ...
            if (discardableRes == 0 && !firstHouse && !firstPath)
            {
                if (discardableRes == 0)
                {
                    if (allClaimed())
                    {
                        ...
                        if (diceroll == 7)
                        {
                            if (wood + grain + wool + ore + brick > 7)
                            {
                                ...
                                thiefMode = true;
                                Debug.LogFormat("[Settlers Of KTaNE #{0}] 7 Rolled. You have {1} ressources. You have to discard {2} rossources.", moduleId, wood + grain + wool + ore + brick, discardableRes);
                            }
                            counter++;
                        }
                        else
                    ...
                    {
                        Strike(LeftDice);
                        Debug.LogFormat("[Settlers Of KTaNE #{0}] STRIKE due to: Not all ressources have been claimed yet. Don't waste!", moduleId);
                ...
                {
                    Strike(LeftDice);
                    Debug.LogFormat("[Settlers Of KTaNE #{0}] STRIKE due to: Not enough ressources have been discarded yet!", moduleId);
                }
            }
            else
            {
                Strike(LeftDice);
                Debug.LogFormat("[Settlers Of KTaNE #{0}] STRIKE due to: Not all rsources have been discarded yet OR haven't build first street or settlement.", moduleId);
            }
    ...
    {
        if (!isSolved|| (IsNoSolveMode && points < 5))
        {
    ...
    void Init()
    {
        RandomiseHex();             //Makes random ressource Hexes
    }

    void RandomiseHex()
    {
        int HexNumber0 = bomb.GetPortCount() + bomb.GetIndicators().Count() + 2;
        if (HexNumber0 == 7)
        {
    ...
    int getRidofSeven(int x) {              
        switch (x) {
            case 0: x = correctNumber(7 + bomb.GetBatteryCount());                              //tr add Batteries
                if (x == 7)
                {
                    return 10;
                }
    ...
    private readonly string TwitchHelpMessage = "Roll the dice with !{0} roll or !{0} roll until <no1> <no2> <...>" +
        ...
        " Claim one or multiple ressources with !{0} hex <hexCardinal> <hexCardinal> <...>|" +
        " Trade resources using !{0} trade <res> <res>. e.g. !{0} trade brick wood: trades 4 brick for 1 wood. Valid ressources are BRICK, WOOD, ORE, GRAIN, WOOL.|" +
        " Discard resources durin thiefmode with !{0} discard <ress> <number>. e.g. !{0} discard brick 4, discards 4 brick.|" +
        " Valid Cardinals are N,NE,SE... & T,TR,BR..." +
    ...
    Hex TPStringtoHex(String partCommand)
    {
        switch (partCommand)
        {
            ...
            case "nw":
                command = command.Substring(2).Trim();
                return theHexes[6];
            case "m": command = command.Substring(1).Trim();
                return theHexes[0];
            case "tr":
                command = command.Substring(2).Trim();
                return theHexes[1];
            case "r": command = command.Substring(1).Trim();
                return theHexes[2];
            case "br":
                command = command.Substring(2).Trim();
                return theHexes[3];
            case "bl":
                command = command.Substring(2).Trim();
                return theHexes[4];
            case "l": command = command.Substring(1).Trim();
                return theHexes[5];
            case "tl":
                command = command.Substring(2).Trim();
                return theHexes[6];
    ...
    KMSelectable[] ProcessTwitchCommand(string TPcommand)
    {
        ...
        try
        {
                ...
                if (ArrayHasDiceNumber(myArray))
                {
                    while (true)
                    {
                        for (int i = 0; i < myArray.Count(); i++)
                        {
                            if (tempCounter < 30)
                            {
                                if (diceRolls[tempCounter] == myArray.ElementAt(i))
                                {
                                    return Enumerable.Repeat(LeftDice, tempCounter - counter + 1).ToArray();
                                }
            ...
            else if (Regex.IsMatch(command, @"^hex +[a-z+ +]+$") || Regex.IsMatch(command, @"^hex +[1-7 +]+$"))                           //collect ressources !# hex [1-7] or !# hex [cardinal]
            {
                command = command.Substring(4).Trim();
            ...
            else if (Regex.IsMatch(command, @"^build +[a-z|0-9| ]*$"))
            {                                                                                       //upgrade or build settlement !# set house/city/settlement ([cardinal/numberReadingorder]of Hex) ([cardinal/numberCW]of that Hex)
                command = command.Substring(6).Trim();
                if (Regex.IsMatch(command, @"^house +[a-z|0-9| ]*$"))
                {
                    command = command.Substring(6).Trim();
                    return new[] { TPStringtoHex(command.Substring(0, 2).Trim()).nearby[TPStringtoInt(command)].SelectObject };
                }
                else if (Regex.IsMatch(command, @"^city +[a-z|0-9| ]*$"))
                {
                    command = command.Substring(4).Trim();
                    return new[] { TPStringtoHex(command.Substring(0, 2).Trim()).nearby[TPStringtoInt(command)].SelectObject };
                }
                else if (Regex.IsMatch(command, @"^settlement +[a-z|0-9| ]*$"))
                {
                    command = command.Substring(11).Trim();
                    return new[] { TPStringtoHex(command.Substring(0, 2).Trim()).nearby[TPStringtoInt(command)].SelectObject };
                }
                else if (Regex.IsMatch(command, @"^street +[a-z|0-9| ]*$"))
                {
                    command = command.Substring(7).Trim();
                    int temp = TPStringtoIntStreet(command);
                    return new[] { Streets[temp] };
                }
            }
            ...
            else if (Regex.IsMatch(command, @"^discard +[a-z]+ +\d*$"))                                                                      //discard ressources
            {
                ...
                if (command != "")
                {
                    temp = new KMSelectable[int.Parse(command)];
